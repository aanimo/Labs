...
function fetchFromGoogleBooks(query, callback) {
    ...
    fetchData(url, (err, data) => {
        if (err || !data || !data.items) {
            return callback(err || new Error('Нет результатов из Google Books'), null);
        }
...
function fetchFromOpenLibrary(query, callback) {
    ...
    fetchData(url, (err, data) => {
        if (err || !data || !data.docs) {
            return callback(err || new Error('Нет результатов Open Library'), null);
        }
...
app.get('/search', (req, res) => {
    ...
    fetchFromGoogleBooks(query, (err, googleBooks) => {
        ...
        console.warn('Пробуем обратиться к Open Library т.к. Google Books API недоступен');
